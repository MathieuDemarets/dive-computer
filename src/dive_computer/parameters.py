...
class Parameters:
    def __init__(self, yaml_path: str, referential_path: str):
        ...
        assert os.path.exists(
            referential_path
        ), f"Referential file not found: {referential_path}"
    ...
    def setup_check(self):
        ...
        def check_values(param_dict, ref_dict, key, full_key):
            ...
            assert (
                param_dict[key] in ref_dict[key]["values"]
            ), f"Invalid value '{param_dict[key]}' for key '{full_key}' (allowed: {ref_dict[key]['values']})"
            ...
            for key, ref_value in ref_dict.items():
                if isinstance(ref_value, dict) and "values" in ref_value:
                    check_values(
                        ...
                        ref_dict,
                        key,
                        key if not full_key else full_key + "." + key,
                    )
