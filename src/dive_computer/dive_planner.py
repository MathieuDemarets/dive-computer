...
class DivePlanner:
    def __init__(
        ...
        self._dive_log = pd.DataFrame(
            columns=[
                ...
                "Start Pressure (bar)",
                "End Pressure (bar)",
                "Start Remaining Pressure (bar)",
                "End Remaining Pressure (bar)",
                "Start ppO2 (bar)",
                "End ppO2 (bar)",
                "Consumption (L)",
                "Total Consumption (L)",
    ...
    def dive_log_append(self, new_log: pd.DataFrame):
        ...
        assert all(
            col in new_log.columns for col in self._dive_log.columns
        ), "new_log must have the same columns as the existing dive log"
    ...
    def ppO2_at_depth(self, depth: float) -> float:
        ...
        return pressure * self.parameters.get_parameter(
            ["attributes", "bottle", "type", "ppO2"]
        )
    ...
    def test_depth_safe(self, depth: float):
        ...
        """
        assert (
            self.ppO2_at_depth(depth) <= self.max_ppo2
        ), f"ppO2 at depth {depth}m ({self.ppO2_at_depth(depth):.2f} bar) exceeds max ppO2 of {self.max_ppo2} bar"
        assert (
            depth <= self.max_depth
        ), f"Depth {depth}m exceeds max depth of {self.max_depth}m (for certification {self.parameters.get_parameter(['certification'])})"
    ...
    def add_dive_step(
        self, step_type: str, time: int, end_depth: float, warning: bool = True
    ):
        """
        ...
        if self.parameters is not None:
            accepted_step_types = self.parameters.get_parameter(
                ["accepted_inputs", "dive_planner", "dive_log", "type"]
            )
            assert (
                step_type in accepted_step_types
            ), f"Invalid step type: {step_type}. Accepted step types: {accepted_step_types}"
        step = len(self._dive_log)
        if step == 0:
            self.dive_log_append(
                pd.DataFrame(
                    [
                        [
                            step,
                            ...
                            0,
                            0,
                            self.pressure_at_depth(0),
                            self.pressure_at_depth(0),
                            self.bottle.pressure,
                            self.bottle.pressure,
                            self.ppO2_at_depth(0),
                            self.ppO2_at_depth(0),
                            0,
                            0,
                            0,
        ...
        start_remaining_pressure = self._dive_log["End Remaining Pressure (bar)"].iloc[
            -1
        ]
        ...
        consumption_L = self.consumption_level(
            start_time, end_time, start_depth, end_depth, unit="L"
        )
        total_consumption_L = start_total_consumption_L + consumption_L
        consumption_bar = self.consumption_level(
            start_time, end_time, start_depth, end_depth, unit="bar"
        )
        ...
        if warning:
            ...
            assert (
                start_remaining_pressure - self.bottle.reserve >= consumption_bar
            ), f"Not enough gas for step {step} ({step_type}). Remaining pressure: {start_remaining_pressure:.2f} bar, Consumption: {consumption_bar:.2f} bar, Reserve: {self.bottle.reserve:.2f} bar"
            ...
            pd.DataFrame(
                [
                    [
                        ...
                        start_pressure,
                        end_pressure,
                        start_remaining_pressure,
                        start_remaining_pressure - consumption_bar,
                        start_ppO2,
                        end_ppO2,
                        consumption_L,
                        total_consumption_L,
                        consumption_bar,
                        total_consumption_bar,
                    ]
                ],
                columns=self._dive_log.columns,
            )
    ...
    def add_automatic_transitions(self, dive_steps: list) -> list:
        ...
        for i in range(len(dive_steps)):
            if i == 0 and dive_steps[i][0] != "Descent":
                ...
                if (
                    dive_steps[i][0] == "Ascent"
                    and dive_steps[i][2] == self.safety_stop_depth
                ):
                    ascent_to_safety_i = i
                elif (
                    dive_steps[i][0] == "Safety Stop"
                    and dive_steps[i][2] == self.safety_stop_depth
                ):
                    safety_stop_i = i
                ...
                if (
                    dive_steps[i][0] in ("Level", "Bottom")
                    and dive_steps[i][2] < dive_steps[i - 1][2]
                ):
                    final_dive_steps.append(
                        ("Ascent", transition_time, dive_steps[i][2])
                    )
                    final_dive_steps.append(dive_steps[i])
                elif (
                    dive_steps[i][0] in ("Level", "Bottom")
                    ...
                    final_dive_steps.append(
                        ("Descent", transition_time, dive_steps[i][2])
                    )
                    final_dive_steps.append(dive_steps[i])
                else:
                    final_dive_steps.append(dive_steps[i])
        for k in add_before_end:
            if (
                dive_steps[k][0] in ("Level", "Bottom")
                and dive_steps[k][2] < dive_steps[k - 1][2]
            ):
                ...
                final_dive_steps.append(dive_steps[k])
        final_dive_steps.append(
            ("Ascent", transition_time, self.safety_stop_depth)
            if ascent_to_safety_i is None
            else dive_steps[ascent_to_safety_i]
        )
        final_dive_steps.append(
            ("Safety Stop", self.safety_stop_time, self.safety_stop_depth)
            if safety_stop_i is None
            else dive_steps[safety_stop_i]
        )
        final_dive_steps.append(
            ("Ascent", transition_time, 0)
            if ascend_to_surface_i is None
            else dive_steps[ascend_to_surface_i]
        )
        return final_dive_steps

    def make_dive_plan(
        self, dive_steps: list, automatic_transitions: bool = True, excel: str = None
    ) -> str:
        """
        ...
        final_dive_steps = (
            self.add_automatic_transitions(dive_steps)
    ...
    def visualize_dive_plan(self, save_path: str = None):
        ...
        for i in range(len(self.dive_log)):
            ax1.plot(
                [
                    self.dive_log["Start Time (min)"].iloc[i],
                    self.dive_log["End Time (min)"].iloc[i],
                ],
                [
                    -self.dive_log["Start Depth (m)"].iloc[i],
                    -self.dive_log["End Depth (m)"].iloc[i],
                ],
                color="blue",
                linewidth=2,
            )
            ax2.plot(
                ...
                [
                    self.dive_log["Start Remaining Pressure (bar)"].iloc[i],
                    self.dive_log["End Remaining Pressure (bar)"].iloc[i],
                ],
                color="green",
                linewidth=2,
            )
        ax2.axhline(
            y=self.bottle.reserve,
            color="purple",
            linestyle="--",
            label="Reserve Pressure",
        )
        ax2.axhline(
            y=self.bottle.pressure, color="black", linestyle="--", label="Full Pressure"
        )
        ax1.axhline(
            y=-self.safety_stop_depth,
            color="orange",
            linestyle="--",
            label="Safety Stop Depth",
        )
        ...
        ax2.set_xticks(
            list(
                set(
                    self.dive_log["Start Time (min)"].tolist()
                    + self.dive_log["End Time (min)"].tolist()
                )
            )
        )
